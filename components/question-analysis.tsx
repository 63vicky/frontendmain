...
export default function QuestionAnalysis({ examId, attemptId }: QuestionAnalysisProps) {
  ...
  const fetchAnalyticsData = async () => {
    try {
      ...
      if (examId) {
        url = `${API_URL}/question-analytics/exam/${examId}`
      } else if (attemptId) {
        url = `${API_URL}/question-analytics/attempt/${attemptId}`
      } else {
      ...
      toast({
        title: "Error",
        description: "Failed to fetch question analytics data",
        variant: "destructive"
      })
    } finally {
      setLoading(false)
